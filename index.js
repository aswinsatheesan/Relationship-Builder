...
app.post("/person", async (req, res) => {
  ...
  app.get("/person/:id", async (req, res) => {
    try {
      ...
      const person = await pool.query(
        "SELECT * FROM people WHERE pid = $1",
        [id]
      );
  ...
  app.delete("/person/:id", async (req, res) => {
    try {
      ...
      const deleteName = await pool.query(
        "DELETE FROM people WHERE pid = $1",
        [id]
      );
  ...
  app.delete("/relations/:id", async (req, res) => {
    try {
      ...
      const deleteTag = await pool.query(
        "DELETE FROM relations WHERE rid = $1",
        [id]
      );
      const number = await pool.query("SELECT MAX(rid) FROM relations");
      let num = number.rows[0];
  ...
  app.delete("/tags/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const deleteTag = await pool.query("DELETE FROM tags WHERE tid = $1", [
        id,
      ]);
